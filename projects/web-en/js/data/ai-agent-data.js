...
// Flashcards + Quiz + Bài học chuyên sâu về AI Agent, Token, Quota, AI at Edge

// 1. Khái niệm AI Agent cho flashcard
window.aiAgentConcepts = [
  // Agent fundamentals
  { en: "AI Agent", vi: "Tác tử AI", category: "agent", definition: "Hệ thống AI tự chủ có khả năng nhận thức (perceive), suy luận (reason), lập kế hoạch (plan) và hành động (act) để đạt mục tiêu.", example: "Agent đặt vé máy bay: hiểu yêu cầu → tìm chuyến → đặt vé → gửi email xác nhận." },
  { en: "LLM Agent", vi: "Agent dựa trên LLM", category: "agent", definition: "Agent dùng LLM làm 'bộ não' để suy luận, đưa ra quyết định và điều phối các công cụ thực thi.", example: "Claude, GPT với tool use — LLM quyết định gọi tool nào, đọc kết quả, rồi hành động tiếp." },
  { en: "ReAct", vi: "Suy luận và hành động", category: "agent", definition: "Pattern (Reason + Act): agent luân phiên suy luận bằng lý luận (Thought), chọn hành động (Action), quan sát kết quả (Observation) rồi lặp lại.", example: "Thought: cần tra thời tiết → Action: call weather API → Observation: trời mưa → Answer: mang ô." },
  { en: "Tool Use", vi: "Sử dụng công cụ", category: "agent", definition: "Khả năng agent gọi các hàm/API bên ngoài (search, calculator, database) qua các tool được khai báo cho model.", example: "Agent hỏi LLM dùng tool 'get_weather(city)' để lấy dữ liệu thời tiết thực tế." },
  { en: "Function Calling", vi: "Gọi hàm", category: "agent", definition: "Cơ chế API cho phép model trả về cấu trúc JSON gọi một hàm cụ thể kèm tham số, thay vì chỉ trả văn bản.", example: "Model trả về {\"name\": \"get_weather\", \"arguments\": {\"city\": \"Hanoi\"}} để app thực thi." },
  { en: "Agentic Loop", vi: "Vòng lặp agent", category: "agent", definition: "Vòng lặp lặp đi lặp lại: LLM sinh quyết định → thực thi tool → đưa kết quả về LLM → LLM sinh quyết định tiếp cho đến khi hoàn thành mục tiêu.", example: "Agent tự lặp 5-10 vòng để giải quyết một yêu cầu phức tạp như 'phân tích và tổng hợp báo cáo'." },
  { en: "Planning", vi: "Lập kế hoạch", category: "agent", definition: "Khả năng agent chia mục tiêu lớn thành chuỗi bước nhỏ, xác định thứ tự và điều kiện thực hiện.", example: "Mục tiêu 'tổ chức buổi họp' → plan: tìm phòng → mời người → chuẩn bị agenda." },
  { en: "Multi-Agent", vi: "Đa agent", category: "agent", definition: "Kiến trúc nhiều agent chuyên biệt phối hợp với nhau, mỗi agent đảm nhận một vai trò, giao tiếp qua message/queues.", example: "Orchestrator agent điều phối: researcher agent tìm tài liệu → writer agent viết → reviewer agent kiểm tra." },
  { en: "Agent Memory", vi: "Bộ nhớ agent", category: "agent", definition: "Khả năng agent lưu và truy xuất thông tin qua các vòng lặp: short-term (context window), long-term (vector DB, files), working memory (session state).", example: "Agent nhớ sở thích người dùng từ các hội thoại trước để gợi ý phù hợp hơn." },
  { en: "RAG", vi: "Truy xuất tăng cường sinh", category: "agent", definition: "Kỹ thuật tìm kiếm tài liệu liên quan từ knowledge base (vector DB) rồi đưa vào context để LLM trả lời chính xác, giảm hallucination.", example: "Chatbot nội bộ: tìm tài liệu công ty → trả lời dựa trên tài liệu thay vì 'bịa'." },
  { en: "Hallucination", vi: "Ảo giác", category: "agent", definition: "Hiện tượng LLM sinh ra thông tin sai, bịa đặt nhưng trình bày rất tự tin; giảm bằng RAG, grounding, tool verification.", example: "LLM khẳng định 'Đà Nẵng là thủ đô của Việt Nam' — hoàn toàn sai nhưng nghe rất thuyết phục." },
  { en: "Orchestrator", vi: "Điều phối viên", category: "agent", definition: "Agent/component trung tâm quyết định agent nào thực hiện task nào, tuần tự hay song song, và tổng hợp kết quả cuối.", example: "Orchestrator nhận 'viết bài blog' → giao outline cho planner, giao draft cho writer, giao review cho editor." },

  // Tools & MCP
  { en: "MCP", vi: "Giao thức Model Context Protocol", category: "tools", definition: "Giao thức mở chuẩn hóa cách LLM kết nối với tools và data sources (resources, tools, prompts) — 'USB-C cho AI'.", example: "Một MCP server duy nhất cung cấp 20 tools GitHub cho mọi LLM mà không cần viết lại code." },
  { en: "Tool Schema", vi: "Khai báo tool", category: "tools", definition: "Định nghĩa JSON mô tả tool: tên, mô tả, tham số (JSON Schema) để LLM hiểu khi nào và gọi thế nào.", example: "{name: 'get_weather', description: 'Lấy thời tiết theo thành phố', parameters: {type: 'object', properties: {city: {type: 'string'}}}}" },
  { en: "Tool Choice", vi: "Chọn tool", category: "tools", definition: "Cơ chế điều khiển model có được gọi tool hay không: auto (tự quyết), required (bắt buộc), none (chỉ chat).", example: "Tool choice = 'required' khi muốn model luôn dùng tool để không trả lời bừa." },
  { en: "Agent Framework", vi: "Framework agent", category: "tools", definition: "Thư viện giúp xây agent nhanh: vòng lặp, tool registry, memory, orchestration có sẵn.", example: "LangChain, LlamaIndex, CrewAI, OpenAI Agents SDK, Google ADK." },
  { en: "Tool Registry", vi: "Sổ đăng ký tool", category: "tools", definition: "Nơi tập trung đăng ký danh sách tool, kiểm tra schema hợp lệ, quản lý quyền gọi và version.", example: "App đăng ký 5 tools, agent chỉ được gọi tool có trong registry và có permission." },
  { en: "Guardrails", vi: "Rào chắn an toàn", category: "tools", definition: "Lớp kiểm soát chặn hành động nguy hiểm/sai của agent: validate input, chặn tool nhạy cảm, huấn luyện model.", example: "Chặn agent gọi tool 'delete_all_data' nếu không có xác nhận hai bước của người dùng." },

  // Token & Context
  { en: "Token", vi: "Token", category: "token", definition: "Đơn vị cơ bản mà LLM xử lý văn bản — có thể là từ, một phần từ (subword) hoặc ký tự; mỗi model có bộ tokenizer riêng.", example: "'AI is fun' ≈ 4 tokens; tiếng Việt có dấu thường tốn nhiều token hơn tiếng Anh cùng nghĩa." },
  { en: "Tokenizer", vi: "Bộ tách token", category: "token", definition: "Thuật toán (thường là BPE/SentencePiece) chia văn bản thành tokens theo từ điển học được từ dữ liệu huấn luyện.", example: "Tiktoken của OpenAI, tokenizer của Llama dùng BPE với vocab ~32k-100k tokens." },
  { en: "Context Window", vi: "Cửa sổ ngữ cảnh", category: "token", definition: "Số token tối đa model nhận trong một lần gọi (input + output); quá giới hạn sẽ lỗi hoặc bị cắt.", example: "Context 128K tokens ≈ 200 trang sách — gồm cả prompt hệ thống, lịch sử chat và câu trả lời." },
  { en: "Input / Output Tokens", vi: "Token đầu vào / đầu ra", category: "token", definition: "Input tokens = prompt gửi lên; output tokens = câu trả lời sinh ra. Nhà cung cấp tính giá khác nhau cho 2 loại này.", example: "Claude: input $3/M, output $15/M — output đắt hơn vì tốn tính toán sinh từng token." },
  { en: "Max Tokens", vi: "Giới hạn token sinh ra", category: "token", definition: "Tham số giới hạn số token output tối đa một lần sinh; nếu câu trả lời dài hơn sẽ bị cắt giữa chừng.", example: "max_tokens=500 → câu trả lời dài 800 tokens bị cắt, kiểm tra finish_reason='length' để xử lý." },
  { en: "Token Estimation", vi: "Ước lượng token", category: "token", definition: "Kỹ thuật ước lượng nhanh số token để tính chi phí và kiểm soát context trước khi gọi API.", example: "Quy tắc gần đúng: tiếng Anh ~1.3 token/từ, 4 ký tự ≈ 1 token; dùng tiktoken để đếm chính xác." },
  { en: "Prompt Caching", vi: "Bộ nhớ đệm prompt", category: "token", definition: "Nhà cung cấp cache phần prefix lặp lại giữa các request, tính giá rẻ hơn nhiều (còn 10%) cho token được cache.", example: "Chat lặp lại system prompt + tài liệu dài → sau lần đầu, phần đó được cache, tiết kiệm tới 90% chi phí." },
  { en: "Context Truncation", vi: "Cắt ngắn ngữ cảnh", category: "token", definition: "Khi vượt context window, hệ thống phải cắt bớt (drop message cũ, tóm tắt, hoặc báo lỗi) — ảnh hưởng đến chất lượng.", example: "Chat dài bị quên tin nhắn đầu vì bị truncate; giải pháp: summary buffer, RAG, sliding window." },

  // Quota & Cost
  { en: "Rate Limit", vi: "Giới hạn tốc độ", category: "quota", definition: "Nhà cung cấp giới hạn số request/khối lượng token trong một khoảng thời gian để bảo vệ hệ thống.", example: "Giới hạn 60 requests/phút hoặc 100K tokens/phút cho tier miễn phí." },
  { en: "RPM / TPM", vi: "Requests / Tokens per minute", category: "quota", definition: "Đơn vị quota: RPM = số request mỗi phút, TPM = số token mỗi phút (input+output) cho phép.", example: "Gói 100 RPM / 500K TPM → mỗi phút tối đa 100 lần gọi và 500K tokens." },
  { en: "Token Bucket", vi: "Xô token", category: "quota", definition: "Thuật toán điều tiết: bucket chứa tokens 'nạp' đều theo thời gian, mỗi request tốn tokens; hết thì chờ.", example: "Bucket 10 tokens, nạp 1/s → bùng nổ 10 request ngay lập tức, sau đó 1 request/s." },
  { en: "Retry & Backoff", vi: "Thử lại và lùi thời gian", category: "quota", definition: "Khi gặp lỗi 429 (rate limit) hoặc 5xx, tự thử lại với thời gian chờ tăng dần (exponential backoff) + jitter.", example: "Lần 1 chờ 1s, lần 2 chờ 2s, lần 3 chờ 4s... tối đa 5 lần, kèm jitter ngẫu nhiên tránh thundering herd." },
  { en: "HTTP 429", vi: "Mã lỗi quá tải", category: "quota", definition: "Too Many Requests — yêu cầu vượt quota; response có header Retry-After để biết chờ bao lâu.", example: "429 + 'Retry-After: 30' → đợi 30s hoặc dùng backoff trước khi gọi lại." },
  { en: "Quota", vi: "Hạn mức", category: "quota", definition: "Tổng tài nguyên (token, request, chi phí) người dùng được phép dùng trong kỳ (ngày/tháng) theo gói đăng ký.", example: "Gói free 1M tokens/tháng; dùng hết phải chờ tháng sau hoặc nâng cấp." },
  { en: "Pricing per Token", vi: "Giá theo token", category: "quota", definition: "Cách tính tiền phổ biến: trả theo số token input và output, khác nhau theo model và tier.", example: "Model A: $1/M input, $10/M output → 10K in + 2K out = $0.01 + $0.02 = $0.03." },
  { en: "Cost Optimization", vi: "Tối ưu chi phí", category: "quota", definition: "Giảm tiền AI bằng cách: prompt ngắn, model rẻ cho task dễ, cache, batch, giảm output token, quản lý context.", example: "Dùng mini model cho classification, big model chỉ cho task khó → giảm 70% chi phí." },
  { en: "Budget & Alert", vi: "Ngân sách và cảnh báo", category: "quota", definition: "Đặt ngân sách tháng, theo dõi chi phí realtime, cảnh báo khi vượt ngưỡng để tránh sốc hóa đơn.", example: "Alert 80% ngân sách → tự chuyển model rẻ hơn hoặc chặn request mới." },

  // AI at Edge
  { en: "Edge AI", vi: "AI tại biên", category: "edge", definition: "Chạy suy luận AI ngay trên thiết bị (điện thoại, laptop, IoT) thay vì gửi lên cloud — giảm latency, tăng privacy, hoạt động offline.", example: "Google Translate chạy offline trên điện thoại, FaceID trên iPhone." },
  { en: "On-Device LLM", vi: "LLM trên thiết bị", category: "edge", definition: "Mô hình ngôn ngữ nhỏ chạy cục bộ trên thiết bị người dùng, không cần internet, dữ liệu không rời máy.", example: "Ollama + Llama 3.2 3B, Gemma 2B, Qwen 0.5B chạy trên laptop/điện thoại." },
  { en: "Quantization", vi: "Lượng tử hóa", category: "edge", definition: "Giảm độ chính xác của trọng số (FP16 → INT8/INT4) để giảm kích thước và tăng tốc suy luận, đánh đổi chút chất lượng.", example: "Model 7B FP16 ≈ 14GB → INT4 ≈ 4GB, chạy được trên máy 8GB RAM." },
  { en: "Model Compression", vi: "Nén mô hình", category: "edge", definition: "Nhóm kỹ thuật làm model nhỏ/nhanh hơn: quantization, pruning (tỉa), distillation (mô hình thầy-trò), low-rank (LoRA).", example: "Distillation: GPT-4 (thầy) dạy model 7B (trò) → model nhỏ học được phần lớn khả năng." },
  { en: "WebGPU", vi: "WebGPU", category: "edge", definition: "API chuẩn cho phép trình duyệt tận dụng GPU để chạy suy luận neural network trực tiếp trên web.", example: "WebLLM, Transformers.js chạy LLM/embedding ngay trong trình duyệt không cần server." },
  { en: "WebAssembly", vi: "WASM", category: "edge", definition: "Binary format chạy tốc độ gần native trên trình duyệt; dùng để chạy tokenizer, mô hình nhỏ, pipeline AI tại client.", example: "Mô hình Whisper nhỏ chạy WASM để transcribe giọng nói 100% trên trình duyệt." },
  { en: "Federated Learning", vi: "Học liên kết", category: "edge", definition: "Huấn luyện mô hình trên nhiều thiết bị, chỉ gửi bản cập nhật gradient về server — dữ liệu không bao giờ rời máy.", example: "Gboard học từ phong cách gõ của người dùng mà không thu thập nội dung gõ về Google." },
  { en: "ML Kit / Core ML", vi: "SDK on-device AI", category: "edge", definition: "SDK của Google (ML Kit) và Apple (Core ML) cho phép chạy model và task AI sẵn có ngay trên mobile.", example: "ML Kit text recognition, Core ML convert PyTorch model sang .mlmodel chạy trên iPhone." },
  { en: "Latency & Privacy", vi: "Độ trễ và quyền riêng tư", category: "edge", definition: "Hai lợi ích chính của edge AI: phản hồi tức thì không cần mạng, và dữ liệu nhạy cảm không gửi lên cloud.", example: "Camera an ninh nhận diện khuôn mặt ngay trên camera thay vì stream video lên server." },
];

// 2. Câu hỏi quiz trắc nghiệm AI Agent
window.aiAgentQuizData = {
  general: [
    { question: "AI Agent khác với chatbot thông thường ở điểm nào?", options: ["Có khả năng tự chủ hành động, gọi tools, lập kế hoạch", "Chỉ trả lời câu hỏi", "Không dùng LLM", "Chạy nhanh hơn"], correct: 0 },
    { question: "Pattern ReAct trong agent nghĩa là gì?", options: ["ReactJS framework", "Luân phiên Reason (Thought) → Action → Observation", "Reactive programming", "Rèn luyện lại mô hình"], correct: 1 },
    { question: "Function Calling cho phép model làm gì?", options: ["Trả về cấu trúc JSON gọi hàm cụ thể kèm tham số", "Tự cài đặt phần mềm", "Gửi email không cần API", "Viết code bằng mọi ngôn ngữ"], correct: 0 },
    { question: "MCP (Model Context Protocol) là gì?", options: ["Một loại model AI", "Giao thức mở chuẩn hóa kết nối LLM với tools và data", "Ngôn ngữ lập trình cho agent", "Một framework UI"], correct: 1 },
    { question: "Vòng lặp agentic (agentic loop) hoạt động thế nào?", options: ["Gọi LLM một lần duy nhất", "LLM quyết định → thực thi tool → đưa kết quả về LLM → lặp lại", "Chạy không cần LLM", "Chỉ gọi API một lần"], correct: 1 },
    { question: "Tác dụng chính của RAG trong agent?", options: ["Làm model chạy nhanh hơn", "Giảm hallucination bằng cách truy xuất tài liệu liên quan đưa vào context", "Tăng kích thước model", "Mã hóa dữ liệu"], correct: 1 },
    { question: "Multi-agent khác single agent ở điểm nào?", options: ["Nhiều agent chuyên biệt phối hợp, mỗi agent một vai trò", "Dùng nhiều GPU hơn", "Chạy nhiều model cùng lúc", "Không dùng LLM"], correct: 0 },
    { question: "Agent Memory chia làm mấy loại chính?", options: ["1 loại", "2 loại", "3 loại: short-term, long-term, working memory", "Không có memory"], correct: 2 },
  ],
  token: [
    { question: "Token trong LLM là gì?", options: ["Đơn vị cơ bản model xử lý văn bản (từ/subword/ký tự)", "Mật khẩu API", "Một loại model", "Đơn vị đo tốc độ mạng"], correct: 0 },
    { question: "Context window là gì?", options: ["Cửa sổ trình duyệt", "Số token tối đa model nhận trong một lần gọi (input + output)", "Kích thước màn hình", "Bộ nhớ RAM"], correct: 1 },
    { question: "Tại sao output tokens thường đắt hơn input tokens?", options: ["Vì output dài hơn", "Vì sinh từng token tốn tính toán hơn đọc prompt", "Vì nhà cung cấp thích vậy", "Vì output cần GPU riêng"], correct: 1 },
    { question: "Prompt caching giúp gì?", options: ["Cache phần prefix lặp lại, giảm chi phí tới 90%", "Làm model nhanh hơn 2 lần", "Tăng context window", "Mã hóa prompt"], correct: 0 },
    { question: "finish_reason='length' nghĩa là gì?", options: ["Câu trả lời bị cắt do vượt max_tokens", "Hoàn thành bình thường", "Lỗi network", "Prompt quá ngắn"], correct: 0 },
    { question: "Ước lượng token gần đúng cho tiếng Anh là gì?", options: ["1 từ ≈ 1.3 token", "1 từ ≈ 10 token", "1 từ ≈ 0.1 token", "1 từ ≈ 1 token chính xác"], correct: 0 },
    { question: "Khi chat dài vượt context window, điều gì xảy ra?", options: ["Model tự mở rộng memory", "Bị truncate/cắt bớt, tin nhắn cũ bị quên", "Model bị crash", "Không có gì xảy ra"], correct: 1 },
  ],
  quota: [
    { question: "RPM và TPM nghĩa là gì?", options: ["Requests và Tokens per minute", "Realtime Processing Model", "Random Path Memory", "Remote Process Manager"], correct: 0 },
    { question: "HTTP 429 nghĩa là gì?", options: ["Internal Server Error", "Too Many Requests — vượt rate limit/quota", "Not Found", "Unauthorized"], correct: 1 },
    { question: "Exponential backoff dùng để làm gì?", options: ["Tăng tốc request", "Thử lại với thời gian chờ tăng dần khi gặp 429/5xx", "Giảm giá API", "Mã hóa request"], correct: 1 },
    { question: "Token Bucket algorithm hoạt động thế nào?", options: ["Bucket chứa tokens nạp đều theo thời gian, request tốn tokens", "Chỉ cho 1 request mỗi giờ", "Không giới hạn gì", "Dùng blockchain"], correct: 0 },
    { question: "Cách tốt nhất khi gặp lỗi 429?", options: ["Gửi lại ngay lập tức nhiều lần", "Đợi theo Retry-After hoặc backoff + jitter rồi thử lại", "Bỏ luôn request", "Đổi API key"], correct: 1 },
    { question: "Làm sao giảm chi phí AI hiệu quả?", options: ["Dùng model rẻ cho task dễ, tối ưu prompt, cache, giảm output", "Gọi API nhiều lần hơn", "Dùng model lớn nhất cho mọi thứ", "Không thể giảm"], correct: 0 },
  ],
  edge: [
    { question: "Edge AI là gì?", options: ["AI chạy trên thiết bị biên (điện thoại, IoT) thay vì cloud", "AI của công ty Edge", "AI chạy ở biên giới", "Một loại model"], correct: 0 },
    { question: "Quantization giúp gì cho on-device AI?", options: ["Giảm kích thước model (FP16 → INT8/INT4) để chạy trên thiết bị nhỏ", "Tăng độ chính xác", "Làm model lớn hơn", "Mã hóa model"], correct: 0 },
    { question: "Lợi ích chính của AI at edge là gì?", options: ["Giảm latency và tăng privacy (dữ liệu không rời máy)", "Rẻ hơn cloud", "Model lớn hơn", "Không cần CPU"], correct: 0 },
    { question: "WebGPU dùng để làm gì?", options: ["Chạy suy luận neural network trong trình duyệt bằng GPU", "Tăng tốc network", "Vẽ đồ họa game", "Lưu trữ data"], correct: 0 },
    { question: "Federated Learning là gì?", options: ["Huấn luyện trên nhiều thiết bị, chỉ gửi gradient về server", "Học trên một máy chủ trung tâm", "Một loại mạng neural", "Không liên quan AI"], correct: 0 },
    { question: "Model 7B FP16 ≈ 14GB, sau quantization INT4 còn bao nhiêu?", options: ["~4GB", "~14GB", "~1GB", "~100GB"], correct: 0 },
  ],
};

// 3. Bài học chuyên sâu AI Agent
window.aiAgentTopics = [
  {
    file: "README.md",
    title: "🤖 AI Agent — Tổng quan",
    content: `Tất tần tật về **AI Agent**: từ khái niệm, kiến trúc, tools, token, quota, chi phí đến AI at Edge.
...
- Làm được mini project agent thực tế`,
    checklist: [
      "Hiểu AI Agent là gì và khác chatbot thế nào",
      "Biết vòng lặp agent (reason → act → observe)",
      "Hiểu token, context window và cách tính chi phí",
      "Biết xử lý rate limit (429, backoff)",
      "Hiểu AI at Edge và on-device LLM"
    ]
  },
  {
    file: "Bai1_Agent_Fundamentals.md",
    title: "📄 BÀI 1 — AGENT FUNDAMENTALS",
    content: `## 1. AI Agent là gì?
...
- [ ] Hiểu ưu/nhược điểm của multi-agent`,
    checklist: [
      "Giải thích được AI Agent khác chatbot thế nào",
      "Vẽ được vòng lặp agentic (Thought → Action → Observation)",
      "Hiểu pattern ReAct và lợi ích của nó",
  ...
  {
    file: "Bai2_Tools_FunctionCalling.md",
    title: "📄 BÀI 2 — TOOLS & FUNCTION CALLING",
    content: `## 1. Tool Use là gì?
...
- [ ] Áp dụng guardrails cho agent`,
  ...
  {
    file: "Bai3_Token_Context.md",
    title: "📄 BÀI 3 — TOKEN & CONTEXT WINDOW",
    content: `## 1. Token là gì?
...
- [ ] Áp dụng kỹ thuật tối ưu token`,
  ...
  {
    file: "Bai4_Quota_RateLimit.md",
    title: "📄 BÀI 4 — QUOTA & RATE LIMIT",
    content: `## 1. Tại sao có rate limit?
...
- [ ] Áp dụng chiến lược tránh rate limit`,
    checklist: [
      "Hiểu RPM, TPM và các đơn vị quota",
      "Giải thích được token bucket algorithm",
      "Biết ý nghĩa các mã lỗi 429, 400, 5xx",
  ...
  {
    file: "Bai5_Cost_Optimization.md",
    title: "📄 BÀI 5 — CHI PHÍ & TỐI ƯU COST",
    content: `## 1. Cách tính giá AI API
...
- [ ] Thiết lập budget và cảnh báo`,
  ...
  {
    file: "Bai6_AI_At_Edge.md",
    title: "📄 BÀI 6 — AI AT EDGE",
    content: `## 1. Edge AI là gì?
...
- [ ] Quyết định được khi nào nên dùng edge`,
    checklist: [
      "So sánh được cloud vs edge AI",
      "Hiểu quantization và mức giảm kích thước",
      "Biết WebGPU/WASM chạy AI trong trình duyệt",
  ...
  {
    file: "Bai7_XayDung_Agent.md",
    title: "📄 BÀI 7 — XÂY DỰNG AGENT THỰC TẾ",
    content: `## 1. Kiến trúc một agent hoàn chỉnh
...
- [ ] Áp dụng best practices an toàn`,
    checklist: [
      "Vẽ được kiến trúc agent hoàn chỉnh",
      "Chọn đúng model và provider cho task",
      "Thiết kế tools với schema rõ ràng",
  ...
  {
    file: "Bai8_Mini_Projects.md",
    title: "📄 BÀI 8 — MINI PROJECTS",
    content: `## Dự án 1 — Chatbot có Function Calling (Cơ bản)
...
4. Có fallback khi model không hiểu tool`,
    checklist: [
      "Làm chatbot function calling cơ bản",
      "Xây RAG agent cho tài liệu nội bộ",
      "Làm agent hỗ trợ coding",
      "Chạy on-device agent với Ollama",
      "Biết cách debug và giới hạn chi phí khi làm project"
    ]
  },
  {
    file: "Bai9_Pho_Bien.md",
    title: "📄 BÀI 9 — TỔNG HỢP PHỎNG VẤN AI AGENT",
    content: `## Câu hỏi phỏng vấn thường gặp
