import{C as e,M as t,N as n,P as r,S as i,g as a,n as o,p as s,u as c}from"./index-BQOrmwx4.js";import{t as l}from"./navigate-edWLrxEl.js";import{t as u}from"./markdown-RpJYUkAQ.js";window.learnTopics=[{file:`README.md`,title:`📚 Học Code — Tổng quan`,content:`Tổng hợp các bài học lập trình cơ bản và nâng cao.
...
- [ ] Biết các thành phần khác: Config Server, Tracing, Message Queue`,checklist:[`Hiểu sự khác biệt Monolithic vs Microservices`,`Cấu hình Service Discovery với Eureka`,`Cấu hình API Gateway với Spring Cloud Gateway`,`Implement Circuit Breaker với Resilience4j`,`Biết các thành phần khác: Config Server, Tracing, Message Queue`]}];var d=window.learnTopics,f=`learnChecklist`;function p(){try{return JSON.parse(localStorage.getItem(f)||`{}`)}catch{return{}}}function m(e){try{localStorage.setItem(f,JSON.stringify(e))}catch{}}function h(e,t,n){let r={...n,[e]:t};return m(r),r}function g(e,t){let n=0,r=0;return e.forEach(e=>{e.checklist&&e.checklist.forEach(e=>{n++,t[e]&&r++})}),{done:r,total:n,percent:n>0?Math.round(r/n*100):0}}function _(e,t){let n={0:{done:0,total:0}};return e.forEach((e,r)=>{let i=r+1,a=e.checklist?e.checklist.filter(e=>t[e]).length:0,o=e.checklist?e.checklist.length:0;n[i]={done:a,total:o}}),n}var v={name:`CodeLearnPage`,data(){return{topics:d||[],currentIndex:0,checklist:{}}},computed:{currentTopic(){return this.currentIndex===0?this.topics[0]||null:this.topics[this.currentIndex-1]||null},renderedContent(){if(!this.currentTopic)return``;let e=``;return this.currentTopic.content&&(e+=u(this.currentTopic.content)),this.currentTopic.checklist&&this.currentTopic.checklist.length>0&&(e+=`<h3>📝 Checklist</h3>`,this.currentTopic.checklist.forEach(t=>{let n=!!this.checklist[t],r=t.replace(/"/g,`&quot;`);e+=`
            <label class="checklist-item ${n?`checked`:``}">
              <input type="checkbox" ${n?`checked`:``} data-item="${r}">
              <span>${t}</span>
            </label>`})),e},progress(){return g(this.topics,this.checklist)},topicStats(){return _(this.topics,this.checklist)}},mounted(){this.checklist=p(),this.$nextTick(()=>{this.bindChecklistEvents()})},updated(){this.$nextTick(()=>{this.bindChecklistEvents()})},methods:{handleNavigate(e){l(e)},selectTopic(e){this.currentIndex=e,window.scrollTo({top:0,behavior:`smooth`})},bindChecklistEvents(){document.querySelectorAll(`.content-body input[type="checkbox"]`).forEach(e=>{e._bound||(e._bound=!0,e.addEventListener(`change`,()=>{this.handleToggle(e.dataset.item,e.checked)}))})},handleToggle(e,t){this.checklist=h(e,t,this.checklist);let n=document.querySelector(`input[data-item="${e}"]`)?.closest(`.checklist-item`);n&&n.classList.toggle(`checked`,t)}}},y={class:`page-root`,style:{"--color-accent":`#1565c0`}},b={class:`container`},x={class:`page-topbar`},S={class:`topbar-right`},C={class:`progress-bar-wrap`},w={class:`progress-track`},T={class:`progress-label`},E={class:`main-layout`},D={class:`sidebar`},O={class:`topic-list`},k=[`onClick`],A={class:`topic-stat`},j={class:`content-area`},M={class:`content-card`},N=[`innerHTML`];function P(o,l,u,d,f,p){return i(),a(`div`,y,[s(`div`,b,[s(`header`,x,[l[2]||=s(`div`,{class:`topbar-left`},[s(`h1`,null,`💻 Học Code`),s(`p`,null,`Học lập trình từ cơ bản đến nâng cao`)],-1),s(`div`,S,[s(`button`,{class:`home-btn`,type:`button`,"aria-label":`Về trang chủ`,title:`Về trang chủ`,onClick:l[0]||=e=>p.handleNavigate(`/`)},`🏠`)])]),s(`div`,C,[s(`div`,w,[s(`div`,{class:`progress-fill`,style:n({width:p.progress.percent+`%`})},null,4)]),s(`span`,T,r(p.progress.done)+` / `+r(p.progress.total)+` bài hoàn thành`,1)]),s(`div`,E,[s(`aside`,D,[l[4]||=s(`h3`,null,`📚 Bài học`,-1),s(`ul`,O,[s(`li`,{class:t([`topic-item`,{active:f.currentIndex===0}]),onClick:l[1]||=e=>p.selectTopic(0)},[...l[3]||=[s(`span`,null,`📋 Danh sách`,-1)]],2),(i(!0),a(c,null,e(f.topics,(e,n)=>(i(),a(`li`,{key:n,class:t([`topic-item`,{active:f.currentIndex===n+1}]),onClick:e=>p.selectTopic(n+1)},[s(`span`,null,r(e.title?.replace(/^📄 /,``)),1),s(`span`,A,r(p.topicStats[n+1]?.done)+`/`+r(p.topicStats[n+1]?.total),1)],10,k))),128))])]),s(`div`,j,[s(`div`,M,[s(`h2`,null,r(p.currentTopic?.title||`Chọn một bài học`),1),s(`div`,{class:`content-body`,innerHTML:p.renderedContent},null,8,N)])])])])])}var F=o(v,[[`render`,P],[`__scopeId`,`data-v-46459392`]]);export{F as default};