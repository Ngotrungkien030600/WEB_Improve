...
- [ ] So sánh được edge vs cloud AI`,checklist:[`Trả lời trôi chảy 10 câu phỏng vấn trên`,`Giải thích được vòng lặp agent bằng ví dụ cụ thể`,`Biết cách xử lý rate limit và tối ưu chi phí`,`So sánh được edge vs cloud AI`]}];var g=window.aiAgentConcepts,_=window.aiAgentQuizData,v=window.aiAgentTopics,y=`aiAgentChecklist`;function b(){try{return JSON.parse(localStorage.getItem(y)||`{}`)}catch{return{}}}function x(e){localStorage.setItem(y,JSON.stringify(e))}function S(e,t){let n=0,r=0;return e.forEach(e=>{e.checklist&&e.checklist.forEach(e=>{n++,t[e]&&r++})}),{done:r,total:n,percent:n>0?Math.round(r/n*100):0}}function C(e,t){let n=e.checklist||[];return{done:n.filter(e=>t[e]).length,total:n.length}}var w=[{id:`learn`,label:`Học`,icon:`📚`},{id:`quiz`,label:`Thi`,icon:`🎯`},{id:`lessons`,label:`Bài học`,icon:`📖`}],T=[{id:`general`,label:`Agent cơ bản`,icon:`🤖`},{id:`token`,label:`Token`,icon:`🔢`},{id:`quota`,label:`Quota`,icon:`⏳`},{id:`edge`,label:`Edge`,icon:`📡`}],E={name:`AiAgentPage`,components:{CHomeTimer:h},data(){return{tabs:w,activeTab:`learn`,filterCategory:`all`,currentIndex:0,isFlipped:!1,selectedLesson:0,concepts:g,lessons:v,checklist:{},quizQuestions:[],quizCategories:T,currentQuestion:0,selectedAnswer:null,answered:!1,quizScore:0,quizStarted:!1,quizCount:10,quizTime:15,quizTimer:null,quizTimeLeft:15,selectedQuizCategory:`general`}},computed:{filteredCards(){return this.filterCategory===`all`?this.concepts:this.concepts.filter(e=>e.category===this.filterCategory)},currentCard(){return this.filteredCards[this.currentIndex]},lessonProgress(){return S(this.lessons,this.checklist)},renderedLesson(){let e=this.lessons[this.selectedLesson];if(!e)return``;let t=e.content?this.renderMarkdown(e.content):``;return e.checklist&&e.checklist.length>0&&(t+=`<h3>📝 Checklist</h3>`,e.checklist.forEach(e=>{let n=!!this.checklist[e],r=e.replace(/"/g,`&quot;`);t+=`
            <label class="checklist-item ${n?`checked`:``}">
              <input type="checkbox" ${n?`checked`:``} data-item="${r}">
              <span>${e}</span>
            </label>`})),t},currentQuizQuestion(){return this.quizQuestions[this.currentQuestion]?this.quizQuestions[this.currentQuestion]:null},quizPercent(){return this.quizScore===0&&!this.quizStarted?`0%`:Math.round(this.quizScore/this.quizQuestions.length*100)+`%`},quizLevel(){let e=this.quizScore/this.quizQuestions.length*100;return e>=90?`🟢 Xuất sắc`:e>=70?`🟡 Tốt`:e>=50?`🟠 Trung bình`:`🔴 Cần cố gắng`}},mounted(){this.checklist=b(),this.$nextTick(()=>this.bindLessonEvents())},updated(){this.$nextTick(()=>this.bindLessonEvents())},methods:{handleNavigate(e){m(e)},filterCategory(){this.currentIndex=0,this.isFlipped=!1},prevCard(){this.currentIndex>0&&(this.currentIndex--,this.isFlipped=!1)},nextCard(){this.currentIndex<this.filteredCards.length-1&&(this.currentIndex++,this.isFlipped=!1)},toggleLessonItem(e,t){this.checklist={...this.checklist,[e]:t},x(this.checklist)},getLessonProgress(e){let t=this.lessons[e];if(!t)return`0/0`;let{done:n,total:r}=C(t,this.checklist);return`${n}/${r}`},bindLessonEvents(){document.querySelectorAll(`.ai-lesson-content .checklist-item input`).forEach(e=>{e._bound||(e._bound=!0,e.addEventListener(`change`,()=>{this.toggleLessonItem(e.dataset.item,e.checked);let t=e.closest(`.checklist-item`);t&&t.classList.toggle(`checked`,e.checked)}))})},startQuiz(){let e=_[this.selectedQuizCategory]||[];if(e.length===0)return;let t=[...e].sort(()=>Math.random()-.5);this.quizQuestions=t.slice(0,Math.min(this.quizCount,e.length)),this.currentQuestion=0,this.selectedAnswer=null,this.answered=!1,this.quizScore=0,this.quizStarted=!0,this.startQuizTimer()},startQuizTimer(){this.quizTimer&&clearInterval(this.quizTimer),this.quizTimeLeft=this.quizTime,this.quizTimer=setInterval(()=>{this.quizTimeLeft--,this.quizTimeLeft<=0&&this.submitQuizAnswer()},1e3)},submitQuizAnswer(){if(this.answered)return;this.answered=!0,this.quizTimer&&clearInterval(this.quizTimer);let e=this.quizQuestions[this.currentQuestion];this.selectedAnswer===e.correct&&this.quizScore++},nextQuestion(){this.answered||this.submitQuizAnswer(),this.currentQuestion<this.quizQuestions.length-1&&(this.currentQuestion++,this.selectedAnswer=null,this.answered=!1,this.startQuizTimer())},resetQuiz(){this.quizTimer&&clearInterval(this.quizTimer),this.quizStarted=!1,this.quizQuestions=[],this.currentQuestion=0,this.selectedAnswer=null,this.answered=!1,this.quizScore=0},renderMarkdown(e){return e?e.replace(/^#{1,6} (.+)$/gm,(e,t)=>`<h${t.length}>${t}</h${t.length}>`).replace(/\*\*(.+?)\*\*/g,`<strong>$1</strong>`).replace(/\n\n/g,`</p><p>`).replace(/^(?!<[h|p|u|o|l|b|c])(.+)$/gm,`<p>$1</p>`).replace(/<p><\/p>/g,``):``}}},D={class:`page-root`,style:{"--color-accent":`#f472b6`}},O={class:`container`},k={class:`ai-topbar`},A={class:`topbar-right`},j={class:`ai-tabs`},M=[`onClick`],N={key:0,class:`ai-section`},P={class:`ai-card-filter`},F={class:`ai-card-inner`},I={class:`ai-card-front`},L={class:`ai-card-category`},R={class:`ai-card-en`},z={class:`ai-card-vi`},B={class:`ai-card-def`},V={class:`ai-card-back`},H={class:`ai-card-ex`},te={class:`ai-card-counter`},U={class:`ai-card-controls`},W={key:1,class:`ai-section`},G={key:0,class:`ai-q-config`},K={class:`ai-q-tabs`},q=[`onClick`],J={class:`ai-q-settings`},Y={class:`ai-q-timer-bar`},X={class:`ai-q-timer-text`},Z={class:`ai-q-card`},Q={class:`ai-q-badge`},ne={class:`ai-q-progress`},re={class:`ai-q-question`},ie={class:`ai-q-options`},ae=[`disabled`,`onClick`],oe={class:`ai-q-stats`},se={class:`ai-q-stat-value`},ce={class:`ai-q-stat-value`},le={class:`ai-q-stat-value`},ue={class:`ai-q-stat-value`},de={key:2,class:`ai-section`},fe={class:`ai-lesson-layout`},pe={class:`ai-lesson-sidebar`},$={class:`ai-lesson-list`},me=[`onClick`],he={class:`ai-lesson-stat`},ge={class:`ai-lesson-progress`},_e={class:`progress-track`},ve={class:`ai-lesson-content`},ye={class:`ai-lesson-card`},be=[`innerHTML`];function xe(ee,m,h,g,_,v){let y=a(`CHomeTimer`);return i(),c(`div`,D,[d(`div`,O,[d(`header`,k,[m[12]||=d(`div`,{class:`topbar-left`},[d(`h1`,null,`🤖 Học AI Agent`),d(`p`,null,`Agent — Tools — Token — Quota — AI at Edge`)],-1),d(`div`,A,[p(y),d(`button`,{class:`home-btn`,type:`button`,"aria-label":`Về trang chủ`,title:`Về trang chủ`,onClick:m[0]||=e=>v.handleNavigate(`/`)},`🏠`)])]),d(`nav`,j,[(i(!0),c(f,null,e(_.tabs,e=>(i(),c(`button`,{key:e.id,class:t([`ai-tab`,{active:_.activeTab===e.id}]),onClick:t=>_.activeTab=e.id},r(e.icon)+` `+r(e.label),11,M))),128))]),_.activeTab===`learn`?(i(),c(`section`,N,[d(`div`,P,[u(d(`select`,{"onUpdate:modelValue":m[1]||=e=>v.filterCategory=e},[...m[13]||=[o(`<option value="all" data-v-39e0821a>📂 Tất cả chủ đề</option><option value="agent" data-v-39e0821a>🤖 Agent Fundamentals</option><option value="tools" data-v-39e0821a>🔧 Tools &amp; MCP</option><option value="token" data-v-39e0821a>🔢 Token &amp; Context</option><option value="quota" data-v-39e0821a>⏳ Quota &amp; Cost</option><option value="edge" data-v-39e0821a>📡 AI at Edge</option>`,6)]],512),[[s,v.filterCategory]])]),d(`div`,{class:t([`ai-card`,{flipped:_.isFlipped}]),onClick:m[2]||=e=>_.isFlipped=!_.isFlipped},[d(`div`,F,[d(`div`,I,[d(`span`,L,r(v.currentCard?.category?.toUpperCase()),1),d(`div`,R,r(v.currentCard?.en),1),d(`div`,z,r(v.currentCard?.vi),1),d(`div`,B,r(v.currentCard?.definition),1)]),d(`div`,V,[m[14]||=d(`span`,{class:`ai-card-category`},`VÍ DỤ`,-1),d(`div`,H,r(v.currentCard?.example),1),m[15]||=d(`p`,{class:`flip-hint`},`🔄 Click để xem lại mặt trước`,-1)])])],2),d(`p`,te,r(_.currentIndex+1)+` / `+r(v.filteredCards.length),1),d(`div`,U,[d(`button`,{class:`ai-card-btn`,onClick:m[3]||=(...e)=>v.prevCard&&v.prevCard(...e)},`⬅️ Trước`),d(`button`,{class:`ai-card-btn`,onClick:m[4]||=e=>_.isFlipped=!_.isFlipped},`🔄 Lật`),d(`button`,{class:`ai-card-btn`,onClick:m[5]||=(...e)=>v.nextCard&&v.nextCard(...e)},`Tiếp ➡️`)])])):l(``,!0),_.activeTab===`quiz`?(i(),c(`section`,W,[_.quizStarted?l(``,!0):(i(),c(`div`,G,[d(`div`,K,[(i(!0),c(f,null,e(_.quizCategories,e=>(i(),c(`button`,{key:e.id,class:t([`ai-q-tab`,{active:_.selectedQuizCategory===e.id}]),onClick:t=>_.selectedQuizCategory=e.id},r(e.icon)+` `+r(e.label),11,q))),128))]),d(`div`,J,[u(d(`select`,{"onUpdate:modelValue":m[6]||=e=>_.quizCount=e},[...m[16]||=[d(`option`,{value:`5`},`5 câu`,-1),d(`option`,{value:`10`},`10 câu`,-1),d(`option`,{value:`15`},`15 câu`,-1)]],512),[[s,_.quizCount,void 0,{number:!0}]]),u(d(`select`,{"onUpdate:modelValue":m[7]||=e=>_.quizTime=e},[...m[17]||=[d(`option`,{value:`10`},`10s`,-1),d(`option`,{value:`15`},`15s`,-1),d(`option`,{value:`20`},`20s`,-1),d(`option`,{value:`30`},`30s`,-1)]],512),[[s,_.quizTime,void 0,{number:!0}]]),d(`button`,{class:`ai-q-start`,onClick:m[8]||=(...e)=>v.startQuiz&&v.startQuiz(...e)},`▶️ Bắt đầu`)])])),_.quizStarted&&v.currentQuizQuestion?(i(),c(f,{key:1},[d(`div`,Y,[d(`div`,{class:`ai-q-timer-fill`,style:n({width:_.quizTimeLeft/_.quizTime*100+`%`})},null,4)]),d(`p`,X,r(_.quizTimeLeft)+`s`,1),d(`div`,Z,[d(`span`,Q,r(_.quizCategories.find(e=>e.id===_.selectedQuizCategory)?.label),1),d(`span`,ne,`Câu `+r(_.currentQuestion+1)+` / `+r(_.quizQuestions.length),1),d(`h2`,re,r(v.currentQuizQuestion.question),1),d(`div`,ie,[(i(!0),c(f,null,e(v.currentQuizQuestion.options,(e,n)=>(i(),c(`button`,{key:n,class:t([`ai-q-option`,{selected:_.selectedAnswer===n,correct:_.answered&&n===v.currentQuizQuestion.correct,wrong:_.answered&&_.selectedAnswer===n&&n!==v.currentQuizQuestion.correct}]),disabled:_.answered,onClick:e=>{_.selectedAnswer=n,v.submitQuizAnswer()}},r(e),11,ae))),128))]),_.answered?(i(),c(`div`,{key:0,class:t([`ai-q-feedback`,_.selectedAnswer===v.currentQuizQuestion.correct?`correct`:`wrong`])},r(_.selectedAnswer===v.currentQuizQuestion.correct?`✅ Chính xác!`:`❌ Chưa đúng. Đáp án đúng: `+v.currentQuizQuestion.options[v.currentQuizQuestion.correct]),3)):l(``,!0),_.answered?l(``,!0):(i(),c(`button`,{key:1,class:`ai-q-submit`,onClick:m[9]||=(...e)=>v.submitQuizAnswer&&v.submitQuizAnswer(...e)},`Xác nhận`)),_.answered&&_.currentQuestion<_.quizQuestions.length-1?(i(),c(`button`,{key:2,class:`ai-q-next`,onClick:m[10]||=(...e)=>v.nextQuestion&&v.nextQuestion(...e)},`Câu tiếp theo ➡️`)):l(``,!0),_.answered&&_.currentQuestion===_.quizQuestions.length-1?(i(),c(`button`,{key:3,class:`ai-q-restart`,onClick:m[11]||=(...e)=>v.resetQuiz&&v.resetQuiz(...e)},`🔄 Làm lại`)):l(``,!0)]),d(`div`,oe,[d(`div`,null,[d(`span`,se,r(_.quizScore),1),m[18]||=d(`span`,{class:`ai-q-stat-label`},`Đúng`,-1)]),d(`div`,null,[d(`span`,ce,r(_.quizQuestions.length),1),m[19]||=d(`span`,{class:`ai-q-stat-label`},`Tổng`,-1)]),d(`div`,null,[d(`span`,le,r(v.quizPercent),1),m[20]||=d(`span`,{class:`ai-q-stat-label`},`Tỉ lệ`,-1)]),d(`div`,null,[d(`span`,ue,r(v.quizLevel),1),m[21]||=d(`span`,{class:`ai-q-stat-label`},`Trình độ`,-1)])])],64)):l(``,!0)])):l(``,!0),_.activeTab===`lessons`?(i(),c(`section`,de,[d(`div`,fe,[d(`aside`,pe,[m[22]||=d(`h3`,null,`📚 Bài học`,-1),d(`ul`,$,[(i(!0),c(f,null,e(_.lessons,(e,n)=>(i(),c(`li`,{key:n,class:t([`ai-lesson-item`,{active:_.selectedLesson===n}]),onClick:e=>_.selectedLesson=n},[d(`span`,null,r(e.title?.replace(/^📄 /,``)),1),d(`span`,he,r(v.getLessonProgress(n)),1)],10,me))),128))]),d(`div`,ge,[d(`span`,null,r(v.lessonProgress.done)+` / `+r(v.lessonProgress.total),1),d(`div`,_e,[d(`div`,{class:`progress-fill`,style:n({width:v.lessonProgress.percent+`%`})},null,4)])])]),d(`div`,ve,[d(`div`,ye,[d(`h2`,null,r(_.lessons[_.selectedLesson]?.title),1),d(`div`,{class:`lesson-body`,innerHTML:v.renderedLesson},null,8,be)])])])])):l(``,!0)])])}var Se=ee(E,[[`render`,xe],[`__scopeId`,`data-v-39e0821a`]]);export{Se as default};