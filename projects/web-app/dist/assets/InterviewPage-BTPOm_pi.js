import{C as e,S as t,c as n,d as r,g as i,h as a,n as o,p as s,s as c,u as l,x as u}from"./index-CA0WqSal.js";import{t as d}from"./navigate-NnOkWMol.js";import{t as f}from"./helpers-dIzySoZn.js";window.interviewTopics=[{file:`README.md`,title:`📚 Java Backend Interview Master Guide — Cheat Sheets`,content:`Tổng hợp 20 cheat sheet (mỗi phần ~1 trang A4) để ôn tập Java Backend Interview.
...
## ✅ CHECKLIST PHẦN 26`,checklist:[`Giải thích JVM Memory Model.`,`Phân biệt các GC algorithms.`,`Dùng profiler để tìm bottleneck.`,`Fix N+1 query, tối ưu index.`,`Cấu hình caching (@Cacheable, Redis).`,`Tuning connection pool.`,`Đọc và phân tích thread dump.`,`Đọc và phân tích heap dump.`,`Tuning Spring Boot application.`]}];var p=window.interviewTopics,m={};async function h(e,t){return t?await window.progressDB.markCompleted(e,`interview`,`checklist`,{title:e}):await window.progressDB.uncomplete(e,`checklist`),m[e]=t,m}function g(e){return e||window.interviewTopics||[]}function _(e){return g()[e]||null}function v(){return m}function y(e){let t=g(e),n=0,r=0;return t.forEach(e=>{e.checklist&&e.checklist.forEach(e=>{n++,m[e]&&r++})}),{done:r,total:n,percent:n>0?Math.round(r/n*100):0}}function b(e){let t=e.checklist?e.checklist.length:0;return{done:e.checklist?e.checklist.filter(e=>m[e]).length:0,total:t}}function x(e){let t=f(e);return t=t.replace(/\*\*\*(.*?)\*\*\*/g,`<strong><em>$1</em></strong>`),t=t.replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`),t=t.replace(/\*(.*?)\*/g,`<em>$1</em>`),t=t.replace(/`([^`]+)`/g,`<code>$1</code>`),t=t.replace(/\[([^\]]+)\]\(([^)]+)\)/g,`<a href="$2">$1</a>`),t}function S(e){if(!e)return``;let t=e.replace(/```([a-zA-Z]*)\n([\s\S]*?)```/g,(e,t,n)=>`<pre><code class="language-${t}">${f(n.trim())}</code></pre>`);return t=t.replace(/(?:^\|[^\n]+\|\n\|\s*[-:]+\s*(?:\|\s*[-:]+\s*)*\|(?:\n\|[^\n]+\|)+)/gm,e=>{let t=e.trim().split(`
`).filter(e=>e.trim());if(t.length<2)return e;function n(e){let t=e.split(`|`);return t.shift(),t.pop(),t.map(e=>e.trim())}let r=`<table><thead><tr>`;n(t[0]).forEach(e=>r+=`<th>${x(e)}</th>`),r+=`</tr></thead><tbody>`;for(let e=2;e<t.length;e++)r+=`<tr>`,n(t[e]).forEach(e=>r+=`<td>${x(e)}</td>`),r+=`</tr>`;return r+=`</tbody></table>`,r}),t=t.replace(/`([^`]+)`/g,`<code>$1</code>`),t=t.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(e,t,n)=>n.endsWith(`.md`)?`<a href="#" class="topic-link" data-topic-file="${n.replace(/^\.\//,``)}">${t}</a>`:`<a href="${n}">${t}</a>`),t=t.replace(/^####\s+(.*)$/gm,`<h4>$1</h4>`),t=t.replace(/^###\s+(.*)$/gm,`<h3>$1</h3>`),t=t.replace(/^##\s+(.*)$/gm,`<h2>$1</h2>`),t=t.replace(/^#\s+(.*)$/gm,`<h1>$1</h1>`),t=t.replace(/\*\*\*(.*?)\*\*\*/g,`<strong><em>$1</em></strong>`),t=t.replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`),t=t.replace(/\*(.*?)\*/g,`<em>$1</em>`),t=t.replace(/^>\s+(.*)$/gm,`<blockquote>$1</blockquote>`),t=t.replace(/^---\s*$/gm,`<hr>`),t=t.replace(/^(\s*)-\s+(.*)$/gm,(e,t,n)=>`<li class="list-depth-${Math.floor(t.length/2)+1}">${n}</li>`),t=t.replace(/^(\s*)\d+\.\s+(.*)$/gm,(e,t,n)=>`<li class="list-depth-${Math.floor(t.length/2)+1}">${n}</li>`),t=t.replace(/(<li[^>]*>.*<\/li>\n?)+/g,`<ul>$&</ul>`),t=t.replace(/\n\n/g,`</p><p>`),t=t.replace(/\n/g,`<br>`),t.startsWith(`<`)||(t=`<p>`+t+`</p>`),t}var C=[{label:`📘 Java Core`,indices:[1,2,3,4,5]},{label:`🗄️ Database & Spring`,indices:[6,7,8,9,10,11,12]},{label:`☁️ DevOps & Architecture`,indices:[13,14,15,16,17,18]},{label:`📝 Practice & Testing`,indices:[19,20,21,22,23,24,25,26,27]},{label:`🔧 Advanced`,indices:[28,29,30,31]},{label:`🛠️ Dev Tools & Infrastructure`,indices:[32,33,34]},{label:`📐 Clean Code & Soft Skills`,indices:[35,36]},{label:`🗄️ NoSQL & Best Practices`,indices:[37,38,39]}];function w(e){return e.replace(/^📄 /,``).replace(/^Phần (\d+) — /i,`$1 — `)}var T={name:`InterviewPage`,data(){return{currentIndex:0,collapsedGroups:Object.fromEntries(C.map(e=>[e.label,!0])),GROUPS:C}},computed:{topics(){return g(p)},currentTopic(){return _(this.currentIndex)},renderedContent(){if(!this.currentTopic)return``;let e=``;if(this.currentTopic.content&&(e+=S(this.currentTopic.content)),this.currentTopic.checklist&&this.currentTopic.checklist.length>0){e+=`<h3>📝 Checklist kiến thức cần nhớ</h3>`;let t=v();this.currentTopic.checklist.forEach(n=>{let r=!!t[n];e+=`
            <label class="checklist-item ${r?`checked`:``}">
              <input type="checkbox" ${r?`checked`:``} data-item="${n.replace(/"/g,`&quot;`)}">
              <span>${n}</span>
            </label>
          `})}return e},progress(){return y(p)}},mounted(){this.$nextTick(()=>{this.bindChecklistEvents()})},updated(){this.$nextTick(()=>{this.bindChecklistEvents()})},methods:{shortTitle:w,goHome(){d(`/`)},selectTopic(e){this.currentIndex=e,this.scrollToTop()},scrollToTop(){let e=this.$refs.cardRef;e&&e.scrollIntoView({block:`start`})},toggleGroup(e){let t=!!this.collapsedGroups[e.label];if(this.collapsedGroups[e.label]=!t,t){let t=e.indices[0];t!==void 0&&this.selectTopic(t)}},getTopic(e){return this.topics[e]||null},getTopicStatus(e){let t=this.getTopic(e);if(!t)return`0/0`;let{done:n,total:r}=b(t);return`${n}/${r}`},bindChecklistEvents(){let e=this.$refs.cardRef;e&&(e.querySelectorAll(`.checklist-item input`).forEach(e=>{e.__vue__||(e.__vue__=!0,e.addEventListener(`change`,()=>{let t=e.dataset.item,n=e.checked;h(t,n),e.closest(`.checklist-item`).classList.toggle(`checked`,n),this.$forceUpdate(),this.scrollToTop()}))}),e.querySelectorAll(`.topic-link`).forEach(e=>{e.__vue__||(e.__vue__=!0,e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.topicFile,r=this.topics.findIndex(e=>e.file===n);r>=0&&this.selectTopic(r)}))}))}}},E={class:`interview-page`},D={class:`interview-container`},O={class:`interview-topbar`},k={class:`topbar-actions`},A={class:`interview-layout`},j={class:`interview-sidebar`},M={class:`topic-list`},N=[`onClick`],P={class:`toggle-icon`},F=[`onClick`],I={class:`topic-status`},L={class:`progress-bar-wrap`},R={class:`progress-text`},z={class:`progress-track`},B={class:`interview-card`,ref:`cardRef`},V={class:`topic-title`},H=[`innerHTML`];function U(o,d,f,p,m,h){return a(),r(`div`,E,[n(`div`,D,[n(`header`,O,[d[2]||=n(`h1`,null,`☕ Java Backend Interview`,-1),n(`div`,k,[n(`button`,{class:`back-btn`,onClick:d[0]||=(...e)=>h.goHome&&h.goHome(...e)},`⬅ Quay lại Trang chủ`)])]),n(`div`,A,[n(`aside`,j,[d[3]||=n(`h3`,null,`📑 Chủ đề`,-1),n(`ul`,M,[n(`li`,{class:u([`topic-item intro-item`,{active:m.currentIndex===0}]),onClick:d[1]||=e=>h.selectTopic(0)},`📋 Tổng quan`,2),(a(!0),r(c,null,i(m.GROUPS,t=>(a(),r(c,{key:t.label},[n(`li`,{class:u([`topic-item group-header`,{collapsed:m.collapsedGroups[t.label]}]),onClick:e=>h.toggleGroup(t)},[n(`span`,P,e(m.collapsedGroups[t.label]?`▶`:`▼`),1),s(` `+e(t.label),1)],10,N),m.collapsedGroups[t.label]?l(``,!0):(a(!0),r(c,{key:0},i(t.indices,t=>(a(),r(`li`,{key:t,class:u([`topic-item group-item`,{active:m.currentIndex===t}]),onClick:e=>h.selectTopic(t)},[n(`span`,null,e(h.shortTitle(h.getTopic(t)?.title||``)),1),n(`span`,I,e(h.getTopicStatus(t)),1)],10,F))),128))],64))),128))]),n(`div`,L,[n(`div`,R,e(h.progress.done)+` / `+e(h.progress.total),1),n(`div`,z,[n(`div`,{class:`progress-fill`,style:t({width:h.progress.percent+`%`})},null,4)])])]),n(`main`,B,[n(`h2`,V,e(h.currentTopic?.title||``),1),n(`div`,{class:`topic-body`,innerHTML:h.renderedContent},null,8,H)],512)])])])}var W=o(T,[[`render`,U],[`__scopeId`,`data-v-810e0276`]]);export{W as default};